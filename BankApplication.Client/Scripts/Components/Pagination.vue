<template>
  <div class="section-pagination" v-if="vmPagination.itemsTotal >= 1">
    <nav aria-label="Page navigation example" class="pagination-navigation">
      <ul v-if="vmPagination.itemsTotal > 0" class="pagination pagination-area">
        ...
        <li
          v-for="page in vmPagination.pages" :key="page"
          :class="['page-item', { active: page == vmPagination.pageNo }]"
        >
          <!-- <a class="page-link" v-if="page == vmPagination.pageNo">{{ page}}</a> -->
        ...
        <li ref="" :class="['page-item',{ disabled: vmPagination.pageNo == vmPagination.pagesTotal,}]">
          <a
            class="page-link"
            @click="onPageNumberClicked(vmPagination.pageNo + 1)"
            href="#"
            aria-label="Next"
          >
